...
dt <- predict(s_pre, dt)
# 
# dat <-  t(as.matrix(dt[, -1]))
# 
# data.harmonized <- combat(dat=dat, batch=batch)
# 
# dt_adj <- as_tibble(t(data.harmonized$dat.combat))
# 
# dt_adj <- bind_cols(Label = dt$Label, dt_adj)
# colnames(dt_adj) <- colnames(dt)
# 
# dt <- dt_adj
# dt <- dt
...
rad_cutoff <- coords(roc_res_train, x = 'best')[1]
...
p_train <- ggboxplot(x = 'Label', y = 'rad_score', data = dt_final_train,
                     add = 'jitter', color = 'Label', palette = 'jco') + ylim(limit_y) + 
...
# dt_nom <- filter(dt_combined_train_final, rad_score > -5 & rad_score < 15)
...
roc_com_train <- pROC::roc(dt_combined_train$Label, res_train_final, 
                           ci = T)
cutoff <- coords(roc_com_train, x = 'best')[1]
...
roc_com_test <- pROC::roc(dt_combined_test$Label, res_test_final,
                          ci = T)
# cutoff <- coords(roc_com_test, x = 'best')[1]
